...
func mustDecodeOAuthState(t *require.Assertions, state string) users.OAuthState {
	var st users.OAuthState
	err := json.Unmarshal(mustBase64Decode(t, state), &st)
	t.NoError(err)
	return st
}

func mustEncodeOAuthState(t *require.Assertions, st users.OAuthState) string {
