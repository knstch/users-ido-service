...
import (
	...
	googlemocks "users-service/internal/connector/google/mocks"
...
func (s *UsersServiceTestSuite) SetupSuite() {
	...
	if cfg.ServiceName == "" {
		cfg.ServiceName = "users-service-test"
	}
	if cfg.JwtSecret == "" {
	...
	s.svc = users.NewService(logger, dbRepo, cfg, s.googleMock, s.rdb)
}
